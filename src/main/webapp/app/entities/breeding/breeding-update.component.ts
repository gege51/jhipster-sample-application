...
@Component({
    selector: 'jhi-breeding-update',
    templateUrl: './breeding-update.component.html'
})
export class BreedingUpdateComponent implements OnInit {
    breeding: IBreeding;
    isSaving: boolean;
    ...
    save() {
        this.isSaving = true;
        if (this.breeding.id !== undefined) {
            this.subscribeToSaveResponse(this.breedingService.update(this.breeding));
        } else {
            this.subscribeToSaveResponse(this.breedingService.create(this.breeding));
        }
    }

    protected subscribeToSaveResponse(result: Observable<HttpResponse<IBreeding>>) {
        result.subscribe((res: HttpResponse<IBreeding>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
    }
